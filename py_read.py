...
def main() -> None:
    ...
    if len(sys.argv) == 1:
        ...
        print(
            "ERROR: INCORRECT INPUT. python3 pySend.py [input_file.json (optional)] [port (optional)]"
        )
        exit()
    ...
    outfile = json.loads(
        out
    )  # convert JSON string to PYTHON dictionary so we can modified it.
    # MODIFIED: ...
    outfile = json.dumps(
        out
    )  # convert it back to JSON string bc write() only accept string
